...
const Home: React.FC = observer(
	(): React.ReactElement => {
		...
		const settingsP = {
			...
			slidesToScroll: 1,
			arrow: true,
			afterChange: (cS: number) => {
				setCurrP(cS + 1)
			},
			beforeChange: (cS: number) => {
				setCurrP(cS + 1)
			},
		}
		const settingsOne = {
		...
		const [mobileP, setMobileP] = React.useState<string[]>([])

		const mobilePortfolio = React.useCallback(() => {
			setMobileP([])
			currentSlide &&
				currentSlide.photos!.forEach((item) => {
					setMobileP((pre) =>
						pre.concat([
							item.photo1!,
							item.photo2!,
		...
		const langChange = (lang: string = "ru") => {
						...
						<div className='portfolio__section'>
							...
							<div id='portfolio__weeding'>
								...
								<div className='slider port__slider__mobil'>
									<SlickSlider {...settingsOne}>
										{mobileP &&
											mobileP!.map((item, idx) => (
												<>
													<div key={idx}>
														<img
															src={`${process.env.REACT_APP_LINK}${item}`}
															alt=''
														/>
													</div>
												</>
